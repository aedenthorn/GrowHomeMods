...
namespace FieldOfView
{
    ...
    public partial class BepInExPlugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            currentFOV = Config.Bind<float>("Options", "CurrentFOV", 60f, "Current field of view.");
        ...
        private void Update()
        {
            ...
            )
            {
                ...
                Dbgl($"new field of view {currentFOV.Value}");
            }
            Camera.main.fieldOfView = currentFOV.Value;
        }

        [HarmonyPatch(typeof(SimpleMove), "Update")]
        static class SimpleMove_Update_Patch
        {
            static void Prefix(SimpleMove __instance)
            {
                if (!modEnabled.Value)
                    return;
                Dbgl($"field of view {__instance.m_TargetFov.Value} {currentFOV.Value}");
                if (
                    (useScrollWheel.Value && Input.mouseScrollDelta.y != 0 && (CheckKeyHeld(modKeyNormal.Value) || CheckKeyHeld(modKeyFast.Value))) ||
                    ((CheckKeyDown(keyIncrease.Value) || CheckKeyDown(keyDecrease.Value)) && (CheckKeyHeld(modKeyNormal.Value, false) || CheckKeyHeld(modKeyFast.Value, false)))
                )
                ...
                __instance.m_TargetFov.SetNow(currentFOV.Value);
